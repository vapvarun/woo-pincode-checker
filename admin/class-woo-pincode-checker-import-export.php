...
class Woo_Pincode_Checker_Import_Export {
	...
	public function ajax_export_pincodes() {
		...
		// Generate filename
		$filename = 'woo-pincodes-export-' . date( 'Y-m-d-His' ) . '.csv';
	...
	public function handle_export_download() {
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] === 'wpc_download_export' ) {
			// Check user capabilities
			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( 'Unauthorized access' );
			}
