 ...
 */
define( 'WOO_PINCODE_CHECKER_VERSION', '1.5.1' );
...
if ( ! defined( 'WPCP_PLUGIN_PATH' ) ) {
	define( 'WPCP_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
}

if ( ! defined( 'WPCP_PLUGIN_URL' ) ) {
...
function wpc_load_plugin_files() {
	...
	// Register activation and deactivation hooks with safe file reference.
	$plugin_file_for_hooks = defined( 'WOO_PINCODE_CHECKER_PLUGIN_FILE' ) ? WOO_PINCODE_CHECKER_PLUGIN_FILE : __FILE__;
	if ( ! empty( $plugin_file_for_hooks ) ) {
		register_activation_hook( $plugin_file_for_hooks, 'activate_woo_pincode_checker' );
		register_deactivation_hook( $plugin_file_for_hooks, 'deactivate_woo_pincode_checker' );
	}
 ...
 *
 * @param string $plugin The plugin basename being activated.
 */
function woo_pincode_checker_activation_redirect_settings( $plugin ) {
	// Get plugin basename safely - avoid calling plugin_basename with empty string.
	$plugin_basename = '';
...
function wpc_check_update_mysql_db() {
	...
	// Check if migration is needed (by version or by checking if critical columns exist).
...
function wpc_update_table_structure( $table_name ) {
	...
	foreach ( $columns_to_add as $column => $alter_query ) {
		...
		if ( empty( $column_exists ) ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- DDL statement with validated table name.
...
function wpc_ensure_table_indexes( $table_name ) {
	...
	$existing_indexes     = $wpdb->get_results( 'SHOW INDEX FROM `' . esc_sql( $table_name ) . '`' );
	$existing_index_names = array();

	foreach ( $existing_indexes as $index ) {
		// phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- MySQL column name.
		$existing_index_names[] = $index->Key_name;
	}

	foreach ( $indexes_to_add as $index_name => $index_query ) {
...
function wpc_add_manual_fix_page() {
	// Use empty string instead of null to avoid PHP 8.3+ deprecation warnings.
	add_submenu_page(
		'', // Hidden menu (empty string instead of null for PHP 8.3+ compatibility).
		'WPC Manual Fix',
		'WPC Manual Fix',
		'manage_options',
		'wpc-manual-fix',
...
function wpc_manual_fix_page_content() {
	...
	if ( isset( $_POST['create_table'] ) ) {
		// Verify nonce.
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'wpc_manual_fix' ) ) {
			...
			<?php if ( $table_exists ) : ?>
				...
				<table class="widefat">
					...
					<tbody>
						...
						<tr>
							<?php // phpcs:disable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- MySQL column names. ?>
							...
							<?php // phpcs:enable WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase ?>
						</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
...
) <?php echo esc_html( $wpdb->get_charset_collate() ); ?> ENGINE=InnoDB;
			</textarea>
		</div>
	</div>
	<?php
...
// Only add filter if we have a valid plugin file.
if ( defined( 'WOO_PINCODE_CHECKER_PLUGIN_FILE' ) && ! empty( WOO_PINCODE_CHECKER_PLUGIN_FILE ) ) {
